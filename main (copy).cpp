...
//WindowClass *StoreWindowInstance(HWND hwnd, WindowClass *(*constructor)(void) );
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow) {
	...
	HWND hMsgHandler = MsgHandler::createWindowInstance().create(0, L"FileTagIndex", WS_OVERLAPPEDWINDOW, 100, 100, 100, 100, HWND_MESSAGE, NULL, hInstance, NULL);
...
//public:
	//static
	/*
	WindowClass *WindowClass::getWindowPtr(HWND hwnd) {
		return (WindowClass *) GetWindowMem(hwnd);
	}
	
	//static
	LRESULT CALLBACK WindowClass::generalWinProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		WindowClass *winPtr = getWindowPtr(hwnd);
		if (winPtr == NULL && msg == WM_CREATE) {
errorf("WM_CREATE");
			if ( ((CREATESTRUCTW *)lParam)->lpCreateParams == NULL) {
				errorf("creating window class without lpParam");
				return -1;
			}
			WinCreateArgs *args = *(WinCreateArgs **) lParam;
			winPtr = args->create(hwnd);
			*((void **) lParam) = args->lpParam;
			//lParam = args->lpParam;
		}
		if (winPtr == NULL) {
			if (msg != WM_CREATE) {
...
//public:
	...
	// static
	WinInstancer MsgHandler::createWindowInstance(void) {
		...
		return WinInstancer(winArgs, helper.winClassName);
	}
	...
	LRESULT CALLBACK MsgHandler::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			...
			case WM_CREATE: //{
				thwnd = TabWindow::createWindowInstance().create(0, L"FileTagIndex", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 100, 100, 500, 500, NULL, NULL, ghInstance, NULL);
...
//public:
	...
	// static
	WinInstancer TabWindow::createWindowInstance(void) {
...
errorf("TabWindow creating Tab");
				dispWindow = thwnd = TabClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD, 0, 0, rect.right, rect.bottom, hwnd, (HMENU) 3, NULL, NULL);
			...
			case WM_COMMAND: //{
				...
				if ((wParam & (256U*256-1)*256*256) == 0 && lParam == 0) {
					...
					case IDM_FILE_MIMAN:
						if (!wHandle[0]) {
							lastoption = 0;
							//DmanClass::createWindowInstance().create(0, L"Manage Directories", WS_VISIBLE | WS_SYSMENU | WS_CAPTION | WS_SIZEBOX | WS_POPUP, 200, 200, 300, 300, hwnd, 0, ghInstance, 0);
							MainIndexManClass::createWindowInstance().create(0, L"Manage Main Indices", WS_VISIBLE | WS_SYSMENU | WS_CAPTION | WS_SIZEBOX | WS_POPUP, 200, 200, 300, 300, hwnd, 0, ghInstance, 0);
						} else {
...
//public:
	...
	// static
	WinInstancer TabClass::createWindowInstance(void) {
	...
	LRESULT CALLBACK TabClass::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			...
			case WM_CREATE: {
				...
				{
					int y1 = 20;
					thwnd = EditSuperClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL, 5, 35, 380, y1, hwnd, (HMENU) 2, NULL, NULL);
					if (!thwnd) {
						errorf("couldn't create editwindow");
					}
					thwnd = EditSuperClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL, 5, 35 + y1 + 5, 380, 300, hwnd, (HMENU) 3, NULL, NULL);
					if (!thwnd) {
						errorf("couldn't create editwindow");
					}
				}
...
errorf("Dman get 1");
						thwnd = MainIndexManClass::createWindowInstance().create(0, L"Select Index", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_POPUP, 100, 100, 500, 500, hwnd, NULL, ghInstance, (LPVOID) &args);
errorf("Dman get 2");
errorf("Dman get 3");
					}
				}
	...
	//					thwnd = ThumbManClass::createWindowInstance().create(0, L"Thumbnail View", WS_VISIBLE | WS_SYSMENU | WS_CAPTION | WS_SIZEBOX | WS_POPUP, 200, 200, 300, 300, hwnd, 0, ghInstance, &args);
						
						if (GetClientRect(hwnd, &rect) == 0) {
							errorf("GetClientRect failed");
						}
						thwnd = ThumbManClass::createWindowInstance().create(0, L"Thumbnail View", WS_VISIBLE | WS_CHILD, 0, 0, rect.right, rect.bottom, hwnd, 0, ghInstance, &args);
...
//public:
	...
	// static
	WinInstancer DmanClass::createWindowInstance(void) {
		...
		switch(msg) {
			case WM_CREATE: //{
				
				{
					...
					thwnd = PageListClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, hwnd, (HMENU) 4, NULL, NULL);
					
					this->plv = (PageListClass *) SendMessage(thwnd, WM_U_PLV, WPARAM_PLV_GETREF, 0);
					if (!this->plv) {
						return -1;
					}
					
					thwnd = StrListClass::createWindowInstance().create(0,  0, WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, hwnd, (HMENU) 3, NULL, NULL);
					
					this->slv = (StrListClass *) SendMessage(thwnd, WM_U_SLV, WPARAM_SLV_GETREF, 0);
					if (!this->slv) {
						return -1;
					}
	...
	//				this->slv->nRows = MAX_NROWS;
			...
			case WM_DESTROY: {
				...
				//! TODO: move to class desctructor
				if (1) {
					thwnd = this->parent;
...
//public:
	...
	// static
	WinInstancer SDmanClass::createWindowInstance(void) {
...
//public:
	...
	// static
	WinInstancer MainIndexManClass::createWindowInstance(void) {
			...
			case 3:
				{
					...
					if (args.inum != 0) {
						thwnd = DmanClass::createWindowInstance().create(0, L"Manage Directories", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_POPUP, 100, 100, 500, 500, hwnd, NULL, ghInstance, (LPVOID) &args);
					}
				}
				break;
			case 4:
				{
					...
					// errorf_old("inum: %llu", args->inum);
					if (args->inum != 0) {
						thwnd = SDmanClass::createWindowInstance().create(0, L"Manage Subdirectories", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_POPUP, 100, 100, 500, 500, hwnd, NULL, ghInstance, (LPVOID) args);
					}
					free(args);
				}
				break;
			...
			case WM_COMMAND: //{
				...
				if (this->option != 2) {
					...
					if (HIWORD(wParam) == 0 && lParam == 0) { // context menu
						...
						TextEditDialogClass::createWindowInstance().create(0, L"Enter Main Index Name", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_POPUP, 150, 150, 500, 500, hwnd, 0, NULL, arg);
...
//public:
	...
	// static
	WinInstancer ThumbManClass::createWindowInstance(void) {
	...
	LRESULT CALLBACK ThumbManClass::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			case WM_CREATE: //{
				...
				ThumbListClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD, 0, SBAR_H, rect.right, rect.bottom-(!!(this->plv->LastPage > 1))*DMAN_BOT_MRG - SBAR_H, hwnd, (HMENU) 3, NULL, NULL);
				PageListClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD, 0, rect.bottom-DMAN_BOT_MRG+1, rect.right, (!!(this->plv->LastPage > 1))*DMAN_BOT_MRG, hwnd, (HMENU) 4, NULL, NULL);
				SearchBarClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD, 0, 0, rect.right, SBAR_H, hwnd, (HMENU) 6, NULL, NULL);
				SetFocus(GetDlgItem(hwnd, 3));
	...
	//						free(this->tlv.ThumbSel), this->tlv.ThumbSel = 0;
						...
						this->option |= 1;
						ViewImageClass::createWindowInstance().create(0, L"View Image", WS_VISIBLE | WS_CHILD, 0, 0, rect.right, rect.bottom, hwnd, (HMENU) 5, NULL, NULL);
						SetFocus(GetDlgItem(hwnd, 5));
						break;
					case 2:	
						...
						thwnd = FileTagEditClass::createWindowInstance().create(0, L"Edit Tags", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_POPUP, 150, 150, 500, 500, hwnd, 0, NULL, NULL);
						SendMessage(thwnd, WM_USER, 0, (LPARAM) this->dnum);
...
//public:
	...
	// static
	WinInstancer PageListClass::createWindowInstance(void) {
...
//public:
	...
	// static
	WinInstancer StrListClass::createWindowInstance(void) {
...
//public:
	...
	// static
	WinInstancer ThumbListClass::createWindowInstance(void) {
...
//public:
	...
	// static
	WinInstancer ViewImageClass::createWindowInstance(void) {
...
//public:
	...
	// static
	WinInstancer FileTagEditClass::createWindowInstance(void) {
	...
	errorf("fte create1");
				...
				EditSuperClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD | WS_VSCROLL | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL, 5, 5, 350, 200, hwnd, (HMENU) 1, NULL, NULL);
...
//public:
	...
	// static
	WinInstancer CreateAliasClass::createWindowInstance(void) {
	...
	LRESULT CALLBACK CreateAliasClass::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			case WM_CREATE: //{
				...
				if (!(this->parent = WindowClass::getWindowMem(thwnd))) {
...
//public:
	...
	// static
	WinInstancer EditSuperClass::createWindowInstance(void) {
		helper.registerWindowClass();
...
//public:
	...
	// static
	WinInstancer SearchBarClass::createWindowInstance(void) {
	...
	LRESULT CALLBACK SearchBarClass::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			case WM_CREATE: //{
				...
				EditSuperClass::createWindowInstance().create(0, L"", WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL, 5, 5, rect.right - 5 - button_x - 5 - 5, textb_h, hwnd, (HMENU) 1, NULL, NULL);
...
//public:
	...
	// static
	WinInstancer TextEditDialogClass::createWindowInstance(void) {
	...
	LRESULT CALLBACK TextEditDialogClass::winProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		switch(msg) {
			case WM_CREATE: //{
				...
				EditSuperClass::createWindowInstance().create(0, L"Default name", WS_VISIBLE | WS_CHILD | WS_VSCROLL | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL, 5, 5, 350, 200, hwnd, (HMENU) 1, NULL, NULL);
...
WindowClass *StoreWindowInstance(HWND hwnd, WindowClass *(*constructor)(void) ) {
	if (constructor == NULL) {
		errorf("constructor is NULL in StoreWindowObject");
		return 0;
	} if (hwnd == 0) {
		errorf("hwnd is 0 in StoreWindowObject");
		return 0;
	}
	if (wMemArray == 0) {
		...
		wMemArrLen = BASE_WMEM_SIZE;
		nwMemWnds = 0;
		
		/*
		wMemArray[0] = malloc(sizeof(struct wMemEntry));
		wMemArray[0]->h = hwnd;
		wMemArray[0]->mem = constructor();
...
errorf("in storewindow3");
	
	if (nwMemWnds == wMemArrLen) {
		struct wMemEntry **twMemArray = calloc(sizeof(struct wMemEntry*)*wMemArrLen*2, 1);
		if (wMemArray == 0) {
			errorf("calloc failed");
			return 0;
		}
		...
		for (i = 0; i < wMemArrLen && wMemArray[i]; i++);
		if (i == wMemArrLen) {
...
void *AllocWindowMem(HWND hwnd, size_t len) {
	...
	if (wMemArray == 0) {
		...
		*/
		...
		for (i = 0; i < wMemArrLen; i++) {
			if (wMemArray[i] != NULL && wMemArray[i]->h == hwnd) {
				errorf("hwnd already mapped to memory");
				return NULL;
			}
		}
...
char FreeWindowMem(HWND hwnd) {
	...
	if (i == wMemArrLen) {
		errorf("couldn't find window memory to free");
		return 1;
	} else {
		free(wMemArray[i]->mem), free(wMemArray[i]), wMemArray[i] = 0;
		nwMemWnds--;
		if (wMemArrLen / 4 >= nwMemWnds && wMemArrLen / 2 >= BASE_WMEM_SIZE) {
			unsigned long long newlen = wMemArrLen / 2;
			struct wMemEntry **twMemArray = calloc(sizeof(struct wMemEntry*)*newlen, 1);
			if (wMemArray == 0) {
				errorf("calloc failed");
				return 1;
			}
			unsigned long long j;
			for (i = 0, j = 0; i < wMemArrLen; i++) {
				if (wMemArray[i]) {
					if (j >= newlen) {
						errorf("winmemarray reduce fatal error");
						exit(1);
					}

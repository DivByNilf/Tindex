...
//	(*cinfo->err->output_message) (cinfo);	// function invocation
	...
	errorf("jpeg exit: %s", buf);

	longjmp(myerr->setjmp_buffer, 1);
}

ImgF *ReadJPG(char *path) {		// test with monochrome jpgs
	char *bmbuf = 0;
	FILE *infile;
	ImgF *image = 0;
	...
	if ((infile = MBfopen(path, "rb")) == NULL) {
	...
	if ((bmbuf = malloc(cinfo.output_width*cinfo.output_height*4)) == 0) {
		errorf("Malloc failed: width %d, height %d", cinfo.image_width, cinfo.image_height);
	}
	image = malloc(sizeof(ImgF));
	image->n = 1;
	image->x = cinfo.image_width;
	image->y = cinfo.image_height;
	image->dur = 0;
	image->img = malloc(sizeof(char *));
	*image->img = bmbuf;
...
ImgF *ReadPNG(char *path) {
	...
	if ((infile = MBfopen(path, "rb")) == NULL) {
		errorf("fopen failed (ReadPNG) \"%s\"", path);
		return 0;
	}
	...
	if (!(color_type == PNG_COLOR_TYPE_RGB || color_type == PNG_COLOR_TYPE_RGB_ALPHA || color_type == PNG_COLOR_TYPE_GRAY || color_type == PNG_COLOR_TYPE_GRAY_ALPHA || PNG_COLOR_TYPE_PALETTE)) {
		errorf("Not RGB, RGBA, grayscale or palette: \"%s\"", path);
	}
	if (bit_depth != 8) {
		if (bit_depth == 16) {
			png_set_strip_16(png_ptr);
			if (!(color_type == PNG_COLOR_TYPE_RGB || color_type == PNG_COLOR_TYPE_RGB_ALPHA)) {
				errorf("non-rgb bit depth 16: %s", path);
			}
		} else {
			errorf("Bit depth not 8, bit_depth: %d, %s", bit_depth, path);
		}
	}
	...
	bmbuf = (LPVOID) malloc(width*height*4);
	if (bmbuf == 0) {
		errorf("Malloc failed: width %d, height %d", width, height);
	}
...
ImgF *ReadGIF(char *path) {		// test with special characters in path
	unsigned char *bmbuf;
	BITMAPINFO bminfo = {0};
	ImgF *image = 0;
	...
	if ((gifFile = DGifOpenFileName(path, &error)) == NULL) {
		errorf("DGifOpenFileName -- Error code1: %d", error);
		return 0;
	}
...
*/
	...
	image->y = height;
	image->dur = delay;
	image->img = malloc(framec*sizeof(char *));
	bmbuf = malloc(res*4);
	
	for (i = 0; i < framec; i++) {
		if (!(image->img[i] = malloc(res*4))) {
			errorf("malloc failed for gif frame %d / %d", i+1, framec);
			while (i > 0) {
				i--;
				free(image->img[i]);
			} free(tcolors), free(delay), free(disposal), free(bmbuf), free(image);
		}
	}
	...
	for (i = 0; i < framec; i++) {
		...
		if (gifFile->SavedImages[i].ImageDesc.ColorMap) {
			...
			errorf("No global colormap or local colormap for %d", i+1);
		}
...
ImgF *ReadBMP(char *path) { //! unfinished
	FILE *infile;
	char buf[50];
	int i, j, k, bpp;
	...
	if (strcmp(buf, "BM") && strcmp(buf, "BA") && strcmp(buf, "CI") && strcmp(buf, "CP") && strcmp(buf, "IC") && strcmp(buf, "PT")) {
		fclose(infile);
		errorf("bitmap init chars, %s", buf);
		return 0;
	}
	...
	if (k == 12) {
		...
		errorf("unsupported bmp header: size - %d", j);
		fclose(infile);
		return 0;
	}
...
ImgF *ReadImage(char *path) {
	...
	breakpath(path, 0, 0, buf);
	
	if (casestrcmp(buf, "jpg") == 0 || casestrcmp(buf, "jpeg") == 0) {
		image = ReadJPG(path);
	} else if (casestrcmp(buf, "png") == 0) {
		image = ReadPNG(path);
	} else if (casestrcmp(buf, "gif") == 0) {
		image = ReadGIF(path);
	} else if (casestrcmp(buf, "bmp") == 0) {
		image = ReadBMP(path);
	} else {
		errorf("no matched file extension: %s", path);
	}
	free(buf);
	if (image == 0) {
		errorf("no picture: %s", path);
	}
	return image;
}
...
ImgF *ResizeImage(ImgF *img, double zoom) {
	...
	dest->img = malloc(dest->n*sizeof(unsigned char*));
...
ImgF *ResizeImageChunk(ImgF *img, double zoom, unsigned long xOut, unsigned long yOut, unsigned long xspos, unsigned long yspos) {
	...
	if (xspos+xOut > full_x || yspos+yOut > full_y) {
		...
		errorf("vals: zoom: %lf, full_x: %llu, full_y: %llu, xspos: %llu, xOut: %llu, yspos: %llu, yOut: %llu", full_x, full_y, xspos, xOut, yspos, yOut);
	}
...
//	ScaleBmChunkBiLin(img->img, dest->img, img->n, img->x, img->y, dest->x, dest->y, xspos, yspos, zoom);
	ScaleBmChunkBiLinCL(img->img, dest->img, img->n, img->x, img->y, dest->x, dest->y, xspos, yspos, zoom);
	return dest;
}
...
errorf("blur1");
	...
	dest->img = malloc(dest->n*sizeof(unsigned char*));
	bm = malloc(dest->n*sizeof(unsigned char*));
	for (pos = 0; pos < dest->n; pos++) {
		dest->img[pos] = malloc(dest->x*dest->y*4);
		bm[pos] = malloc(dest->x*dest->y*4);
	}
	
//	float stdev = 5;
	int range = stdev*3;
	float *mods = malloc((range+1)*sizeof(float));
	float out = 1;
